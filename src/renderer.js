document.addEventListener('DOMContentLoaded', async () => {
  ...
  function tick(timestamp) {
    ...
    if (spriteRenderer) {
        if (physics.state === 'sleeping') {
            spriteRenderer.play('sleep', 4);
        } else if (physics.state === 'jumping' || physics.state === 'falling' || physics.state === 'wall-jumping' || physics.vy > 100 || physics.vy < -50) {
            spriteRenderer.play('jump', 8);
        } else if (Math.abs(physics.vx) > 10 && (physics.state === 'roaming' || physics.state === 'forced-roaming' || physics.state === 'chasing-ball' || physics.state === 'playing')) {
            spriteRenderer.play('run', 12);
        } else {
            spriteRenderer.play('idle', 6);
        }
        spriteRenderer.update(dt);
        let flip = 1;
        if (pet.classList.contains('facing-left')) flip = -1;
        pet.style.transform = `scaleX(${flip})`;
    } else {
